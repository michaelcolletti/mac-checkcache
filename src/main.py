...
def print_dir_tree(path, prefix="", is_last=True, max_depth=3, current_depth=0):
    ...
    try:
        ...
        last_access_str = (
            last_access.strftime("%Y-%m-%d %H:%M:%S") if last_access else "Unknown"
        )
        ...
        print(
            f"{prefix}{connector}{basename} ({format_size(size)}, Last access: {last_access_str}){old_marker}"
        )
...
def analyze_cache_directory(directory):
    ...
    if old_caches:
        ...
        for path, size, last_access in old_caches:
            last_access_str = (
                last_access.strftime("%Y-%m-%d") if last_access else "Unknown"
            )
            print(
                f"- {os.path.basename(path)}: {format_size(size)}, Last access: {last_access_str}"
            )
...
def interactive_cleanup(directories):
    ...
    print("=" * 80)
    print(
        f"Found {len(all_old_caches)} items that haven't been accessed in 12+ months."
    )
    ...
    while True:
        ...
        if choice == "1":
            ...
            for i, (path, size, last_access) in enumerate(all_old_caches):
                ...
                print(
                    f"{i+1}. {path} ({format_size(size)}, Last access: {last_access_str})"
                )
